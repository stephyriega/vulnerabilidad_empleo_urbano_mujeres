...
global data = "$root\3_datos\0_one_panel"
...
use "$data/enaho_1_16.dta", clear
duplicates drop conglome vivienda hogar_16 hogar_17 hogar_18 hogar_19 hogar_20, force
...
forvalues i=16/19 {
    replace panel_`i'=0 if panel_`i'==2
}

egen paneles=rowtotal(hpanel1617 hpanel1718 hpanel1819 hpanel1920)
...
*save "$data/enaho_1_1_16.dta", replace
...
save "$data/enaho_1_`i'_`j'.dta", replace
...
save "$data/enaho_2_`i'_`j'.dta", replace
...
*49 variables
keep num conglome_* vivienda_* hogar_* codperso_* p401c_* p401f_* p401h*_* p401_* p4021_* p4022_* p4023_* p4024_* p4025_* p413b1_* p413b2_* p414_0*_* p414_1*_* p4191_* p4192_* p4193_* p4194_* p4195_* p4196_* p4197_* p4198_* panh_*

*drop if p401c_`i'==. // no responde sobre trabajo, asi q no sabemos
*drop if p401c_`j'==. // no responde sobre trabajo, asi q no sabemos


reshape long conglome_ vivienda_ hogar_ codperso_ p401c_ p401f_ p401h1_ p401h2_ p401h3_ p401h4_ p401h5_ p401h6_ p401_ p4021_ p4022_ p4023_ p4024_ p4025_ p413b1_ p413b2_ p414_01_ p414_02_ p414_03_ p414_04_ p414_05_ p414_06_ p414_07_ p414_08_ p414_09_ p414_10_ p414_11_ p414_12_ p414_13_ p414_14_  p414_15_ p414_16_ p4191_ p4192_ p4193_ p4194_ p4195_ p4196_ p4197_ p4198_ panh_, i(num) j(ano)

save "$data/enaho_3_`i'_`j'.dta", replace
...
reshape long conglome_ vivienda_ hogar_ codperso_ ubigeo_ estrato_ p203a_ p501_ p506r4_ p510a1_ p511a_  p512b_  p512a_ p513t_ p517_ p523_ p524a1_  p530a_ p538a1_ p541a_ p5561a_ p5562a_ p5563a_ p5564a_ p5565a_ p5566a_ p5567a_ p5568a_  p5571a_  p5572a_  p5573a_  p5574a_  p5575a_  p5576a_  p5577a_  p5578a_ p5581a_ p5582a_ p5583a_ p5584a_ p5585a_ p5586a_ p5587a_ p558a1_ p558a2_ p558a3_ p558a4_ p558a5_ p558c_ p559_01_ p559_02_ p559_03_ p559_04_ p560t_01_ p560t_02_ p560t_03_ p560t_04_ p560t_05_ p560t_06_ p560t_07_ p560t_08_ p560t_09_ p560t_10_ p599_ i524a1_ i530a_ i538a1_  d529t_ d536_ d540t_ i541a_ d543_ d544t_ p558e6_ ocu500_ ocupinf_ emplpsec_ fac500a_ panh_, i(num) j(ano)

save "$data/enaho_4_`i'_`j'.dta", replace
...
save "$data/enaho_5_`i'_`j'.dta", replace
...
save "$data/enaho_6_`i'_`j'.dta", replace
...
preserve 
*empleo
use "$data\enaho_4_`i'_`j'.dta", clear
*educacion
merge 1:1 num ano using "$data\enaho_2_`i'_`j'.dta", gen (merge2)
*salud
merge 1:1 num ano using "$data\enaho_3_`i'_`j'.dta", gen (merge3)
*caracteristicas de las personas en la vivienda
merge 1:1 num ano using "$data\enaho_6_`i'_`j'.dta", gen (merge6)
*sumaria
merge m:1 panh_ ano using "$data\enaho_5_`i'_`j'.dta", gen(merge4)
*características de la vivienda
merge m:1 panh_ ano using "$data\enaho_1_`i'_`j'.dta", gen(merge5)

*conglome_ vivienda_ hogar_ ubigeo_ dominio_ estrato_ ano
...
ren (*_) (*)
save "$data\enaho_`i'_`j'.dta", replace
restore
}
